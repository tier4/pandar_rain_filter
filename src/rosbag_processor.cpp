...
int main(int argc, char **argv)
{
  ...
  for(rosbag::MessageInstance const m: rosbag_view)
  {
    ...
    if(m.getTopic() == "/sensing/lidar/top/pointcloud_raw_ex")
    {
    ...
    if(m.getTopic() == "/sensing/lidar/left/pointcloud_raw_ex")
    {
      sensor_msgs::PointCloud2::ConstPtr cur = m.instantiate<sensor_msgs::PointCloud2>();
      if (cur != NULL)
      {
    ...
    if(m.getTopic() == "/sensing/lidar/right/pointcloud_raw_ex")
    {
      sensor_msgs::PointCloud2::ConstPtr cur = m.instantiate<sensor_msgs::PointCloud2>();
      if (cur != NULL)
      {
